...
```{r}
write_delim(uniprot_mapping, "ecoli/uniprot_mapping", delim='\t')
```
...
```{r}
write_delim(ecocyc_mapping, "ecoli/ecocyc_mapping.tsv", delim='\t')
```
...
```{r}
TU_per_gene = TU %>% select(Site, ecocyc = "Genes of transcription unit") %>% separate_rows(ecocyc, sep = ' // ') %>% right_join(ecocyc_mapping) %>% select(bnumber, Site) %>% arrange(bnumber)
```

```{r}
write_delim(TU_per_gene, "ecoli/TU_per_gene.tsv", delim='\t')
```
...
```{r}
pathway_per_gene = pathway %>% select(Pathways, ecocyc = "Genes of pathway") %>% separate_rows(ecocyc, sep = ' // ') %>% right_join(ecocyc_mapping) %>% select(bnumber, Pathways) %>% arrange(bnumber)
```

```{r}
write_delim(pathway_per_gene, "ecoli/pathway_per_gene.tsv", delim='\t')
```
