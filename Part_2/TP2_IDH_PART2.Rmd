...
```{r}
genes = read_tsv("import/ncbi.bnumber.location.tsv")
genes$organism=511145
genes %>% rename(gene_id=bnumber) %>% write_csv("import/ncbi.bnumber.location.csv")
```
...
# On recupere les pairs de gènes dont le score de co-expression est superieur à 400
```{r}
...
#retrieval des pairs co-exprimés
```{r}
...
links.detailed %>%
  ...
  mutate(organism=str_extract(protein1, '^\\d+'), id1=str_extract(protein1, 'b\\d+'), id2=str_extract(protein2, 'b\\d+')) %>%
  select(organism:id2,experimental, neighborhood, textmining, database, combined_score) %>%
  write_csv("import/string_other.csv") # mettre ce fichier dans l'import de Neo4J
```
...
```{r}
'
LOAD CSV WITH HEADERS FROM "file:///string_other.csv" AS line
MATCH (g1:Gene),(g2:Gene)
WHERE g1.id=line.id1 AND g2.id=line.id2
WITH g1,g2, toInteger(line.experimental) AS expe,
toInteger(line.neighborhood) AS neigh, toInteger(line.textmining) AS textm, toInteger(line.database) AS db, toInteger(line.combined_score) AS cs 
MERGE (g1)-[:STRINGdb {experimental: expe, neighborhood: neigh, textmining: textm, database: db, combined_score: cs}]-(g2)
'  %>% cypher
```
...
```{r}
g2 = 'MATCH p = ()-[r:STRINGdb]-() WHERE r.coexpression>=400 OR r.neighborhood>=400 OR r.experimental>=400 RETURN p'  %>% cypher(type="graph")
...
plot(g2i, vertex.label=NA, main="vous l'appelez comme vous voulez")
```
...
keyword.tab %>% head
```
Je convertie un tableau avec une col nbumber et une col keyword
en ligne bnumber et colonne keywords avec true ou false si le mot clé est là
```{r}
