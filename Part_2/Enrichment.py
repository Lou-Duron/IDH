...
try:
	...
	print('Error : An error occurred while trying to connect to the DBMS. Please check your password or that the DBMS is running')
	exit(1)
...
def hypergeometric(c, q, t, g):
	return hypergeom.cdf(q - c, g, int((1 - float(t)/g)*g), q)
...
for count, s in enumerate(sets):
	...
	if not arg.eval: # If we are not evaluating the metrics
		if arg.metric=='binomial':
			...
			measure = hypergeometric(c, q, t, g)
		...
		temp_results.append(hypergeometric(c, q, t, g)) # Hypergeometric
...
if not arg.eval:
	...
	for count, r in enumerate(results):
		...
		elif arg.metric != 'coverage' and r['measure'] > arg.alpha: break # Alpha threshold
		# OUTPUT
	...
	for key in results.keys():
		...
		for count, r in enumerate(results[key]):
			...
			if arg.limit > 0 and count+1>arg.limit: break # Limited output
			elif key != 'Coverage' and r['measure'] > arg.alpha: break # Alpha threshold
			# OUTPUT
			pval = "{:.4f}".format(r['measure']) if r['measure']>0.01 else "{:.2e}".format(r['measure'])
			print(f"{r['id']}\t{pval}\t{r['common.n']}/{r['target.n']}", end=' ')
			if arg.verbose : 
